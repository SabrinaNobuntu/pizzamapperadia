...
@Component({
  selector: 'app-list-opcional',
  templateUrl: './list-opcional.component.html',
  styleUrls: ['./list-opcional.component.scss']
})
export class ListOpcionalComponent implements AfterViewInit,  OnDestroy  {
  ...
  userConfig; 
  searchableFields = [] 
  //apiURL: string = environment.OpcionalAPIPath;//Campo que é alterável, o nome do campo "variations"+"APIPATH". 
  JSONURL: string = environment.opcionalJSONPath;// Campo que é alterável, o nome do campo "variations"+"JSONPATH". 
  /** 
  ...
  ngAfterViewInit(): void { 
    this.formGenerator.getJSONFromDicionario(this.JSONURL).pipe(takeUntil(this.ngUnsubscribe)).subscribe((dicionarioJSON) => { 
      ...
      createdComponent.JSONPath = this.JSONURL; 
    }); 
  } 
