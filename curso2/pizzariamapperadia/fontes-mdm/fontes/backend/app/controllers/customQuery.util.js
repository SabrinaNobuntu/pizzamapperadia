exports.findCustom = (req, res, mongooseModel) => { 
    var condition = {}; 
  ...
  if (req.body.hasOwnProperty('filterValues') == false) { 
    res.status(500).send({ 
      message: "Erro de entrada de parametros ocorreu ao buscar Categories." 
    }); 
    return; 
  } 
  filterValues = req.body.filterValues; 
  filterConditions = req.body.conditions; 

  if (filterValues.length <= 0) { 
    res.status(500).send({ 
      message: "Não contém filtros para ser realizado a busca de Categories." 
    }); 
    return; 
  } 
  ...
  mongooseModel.find(_filterQuerys[0]) 
    .then(data => { 
    ...
    .catch(err => { 
      res.status(500).send({ 
        message: 
          err.message || "Algum erro desconhecido ocorreu ao buscar Categories." 
      }); 
    }); 
} 
